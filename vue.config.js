module.exports = {
            ...
            postcss: {
                plugins: [
                    ...
                    require("postcss-pxtorem")({
                        rootValue: 80, // 换算的基数(设计图750的根字体为32，设计图是1920*1080，则为192;现在电脑是1366的，设置为136，方便)
    ...
    // cli3 代理是从指定的target后面开始匹配的，不是任意位置；配置pathRewrite可以做替换；；；；------设置代理proxy的目的，解决跨域问题
    devServer: {
        port: '8080',
        open: true,
        proxy: {
            '/api': {
                ...
                target: 'http://192.168.1.104:8000',//服务器的真实地址
                // 此处target的意义在于：造成跨域是因为访
                // 问的host与我们的请求头里的origin不一致，所以我们要设置成一致，这个具体请看下文
                changeOrigin: true,
                pathRewrite: {'^/api': 'https://192.168.1.104:8000/api'}
                // 此处是大部分文章都不会明说的的地方，
